...
unsigned int calculateIntsPerVertex(const std::vector<VBOAttribMarker> &markers) {
    unsigned int max = 0;
    for (auto it = markers.begin(); it!= markers.end(); it++) {
        max = std::max(max, static_cast<unsigned int>(it->numElements + it->offset / sizeof(GLint)));
    }
    return max;
}
...
VBO::VBO(const int *data, int sizeInFloats, std::vector<VBOAttribMarker> markers, GEOMETRY_LAYOUT layout) :
    m_handle(-1),
    m_markers(markers),
    m_bufferSizeInFloats(sizeInFloats),
    m_numberOfFloatsPerVertex(calculateIntsPerVertex(markers)),
    m_stride(m_numberOfFloatsPerVertex * sizeof(GLint)),
    m_triangleLayout(layout),
    m_int(true)
{
    // Generate vbo
    ...
    // Bind and send data to vbo
    glBindBuffer(GL_ARRAY_BUFFER, m_handle);
    glBufferData(GL_ARRAY_BUFFER, sizeInFloats * sizeof(GLint), &data[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
