...
//GRANT ADDED
void Shape::addVertex(glm::vec3 position, glm::vec3 normals, glm::vec2 textureCoords){
    addPosition(position.x, position.y, position.z);
    addNormal(normals.x, normals.y, normals.z);
    addTextureCoordinate(textureCoords.x, textureCoords.y);
}


//void Shape::addVertex(const ShapeVertex &vertex){
//    addVertex(vertex.pos, vertex.normal, vertex.texCoords);
//}

void Shape::setLayout(VBO::GEOMETRY_LAYOUT layout){
    m_vboTriangleLayout = layout;
}
