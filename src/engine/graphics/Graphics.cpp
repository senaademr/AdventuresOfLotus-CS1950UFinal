    ...
    m_numLights(0)
{
    ...
    addShader("skinning", ":/shaders/skinning.vert", ":/shaders/shader.frag");
...
void Graphics::drawShape(std::shared_ptr<Shape> shape) {
    ...
    if(!shape->m_built) {
        if(!shape->printDebug()) {
            std::cerr << "The above warnings pertain to shape '" << shape->m_shapeName << "'" << std::endl;
            std::cerr << std::endl;
        }
    }
#endif
...
void Graphics::drawShape(std::shared_ptr<MeshShape> shape) {
#if GRAPHICS_DEBUG_LEVEL > 0
    if(!shape->m_built) {
        if(!shape->printDebug()) {
            std::cerr << "The above warnings pertain to shape '" << shape->m_shapeName << "'" << std::endl;
...
void Graphics::setBones(int index, glm::mat4x4& boneTransform){
    m_activeShader->setUniformArrayByIndex("bones", boneTransform, index);
}
...
void Graphics::setLight(const Light &light, int index) {
    ...
    setUniform("numLights", m_numLights);
    int lightsSize = static_cast<int>(m_lights.size());
    if(lightsSize < m_numLights) {
