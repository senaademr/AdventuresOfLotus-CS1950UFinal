...
void View::initializeGL()
{
    ...
    //m_application = std::make_shared<DungeonCrawlerApplication>(this);
    m_application = std::make_shared<FinalApplication>(this);
    m_application->initializeGL();
}
...
bool View::getCaptureMouse(){
    return m_captureMouse;
}

std::shared_ptr<Application> View::getApplication(){
    return m_application;
}

void View::setCaptureMouse(bool captureMouse){
    m_captureMouse = captureMouse;
    updateMouseTracking();
}

void View::updateMouseTracking(){
    if(m_captureMouse) {
        QApplication::setOverrideCursor(Qt::BlankCursor);
        QCursor::setPos(mapToGlobal(QPoint(width() / 2, height() / 2)));
    }
